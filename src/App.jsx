...
export default function App() {
  ...
  const EMPTY_EQ = {name:"",manufacturer:"",category:"Excavator",weight:"",power:"",length:"",width:"",height:"",trailerType:"RGN / Lowboy",haulerNote:""};
  ...
  async function loadCustom(){
    try{
      const r=await store.list("custom:");
      const items=await Promise.all(r.keys.map(async k=>{
        try{const v=await store.get(k);return v?{_key:k,...JSON.parse(v.value)}:null;}catch{return null;}
      }));
      setCustom(items.filter(Boolean).sort((a,b)=>(b._savedAt||0)-(a._savedAt||0)));
    }catch{setCustom([]);}
  }

  async function saveCustom(eq){
    const key="custom:"+slug(eq.name)+"-"+Date.now();
    await store.set(key,JSON.stringify({...eq,_savedAt:Date.now()}));
    loadCustom();
  }

  async function deleteCustom(key){
    await store.delete(key);loadCustom();setToast("DELETED");
  }

  async function loadPhoto(name){
    try{
      const r=await store.get("photo:"+slug(name));
      if(r?.value){setPhoto(r.value);setImgMode("photo");}
      else{setPhoto(null);setImgMode("diagram");}
    }catch{setPhoto(null);setImgMode("diagram");}
  }
  ...
  async function generateNew(){
    ...
    try{
      ...
      await saveCustom(parsed);setToast("SAVED");openProfile(parsed);setAiQuery("");
  ...
  async function saveManual(){
    ...
    const eq = {
      ...
      transportInfo:{"Trailer Type":newEq.trailerType,"Permits Required":"Verify state requirements"},
  ...
  async function handlePhoto(e){
    ...
    try{
      ...
      await store.set("photo:"+slug(current.name),b64);
      setToast("PHOTO SAVED");
    }catch(err){setToast("Upload failed: "+err.message);}
  }

  async function removePhoto(){
    try{await store.delete("photo:"+slug(current.name));setPhoto(null);setImgMode("diagram");setToast("PHOTO REMOVED");}catch{}
  }

  const isCustom=current&&custom.some(c=>slug(c.name)===slug(current.name));
